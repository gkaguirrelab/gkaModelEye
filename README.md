# gkaModelEye
These routines implement a ray-traced model eye. A primary application of the model is to describe the entrance pupil in the image plane for a rotated eye. The entrance pupil is described by the parameters of an ellipse fit to the pupil perimeter.
...
A good place to get started is to try rendering the model eye for a few views. Examine the examples in the header of the routine `renderEyePose.m`, or give this a try:
```
