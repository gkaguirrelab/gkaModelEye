...
eyeCoord = convertWorldToEyeCoord(worldTarget);

% Counter-rotate the tatget by the eye pose, so that the eyeCoordTarget is
% in a position equivalent to if the eye had rotated.
eyeCoordTarget = rotateEyeCoord(eyeCoord, -eyePose, rotationCenters);
