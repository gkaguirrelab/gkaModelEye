...
%{
    ...
    degField = [5.8 3.0 0];
    G = findRetinaFieldPoint( sceneGeometry.eye, degField);
...
% Convert the x0 zero guess into geodetic coordinates
...
myObj = @(G) sqrt(sum((degField(1:2)-quadric.angleRays(calcNodalRay(eye,G,[],cameraMedium),opticalAxis)).^2));
